...
export default function SellForm({
  ...
  const CATEGORY_FIELDS = {
    ...
    fertilizers: [
      ...
      {
        name: "composition",
        label: t("form_composition"),
        type: "text",
        placeholder: t("ph_composition"),
      },
      {
        name: "description",
        label: t("form_description"),
    ...
    pesticides: [
      ...
      },
      {
        ...
        label: t("form_target"),
        type: "text",
        placeholder: t("ph_target"),
      },
      {
        name: "quantity",
        label: `${t("form_quantity")} (L/kg)`,
        type: "number",
        placeholder: t("ph_quantity"),
      },
      {
    ...
    livestock: [
      ...
      {
        name: "animal_type",
        label: t("form_animal_type"),
        type: "text",
        placeholder: t("ph_animal_type"),
      },
      {
        name: "age",
        label: t("form_age"),
    ...
    oil: [
      ...
      {
        name: "quantity",
        label: `${t("form_quantity")} (litres)`,
        type: "number",
        placeholder: t("ph_litres"),
      },
      {
        name: "price",
        label: `${t("form_price")} (₹)`,
    ...
    services: [
      {
        name: "title",
        label: t("form_service_title"),
        type: "text",
        placeholder: t("ph_service_title"),
      },
      {
        name: "service_type",
        label: t("form_service_type"),
        type: "text",
        placeholder: t("ph_service_type"),
      },
      {
        name: "price",
        label: t("form_price"),
    ...
    others: [
      ...
      {
        name: "price",
        label: `${t("form_price")} (${t("optional")})`,
        type: "number",
        placeholder: t("ph_optional_price"),
      },
    ],
  };
  ...
  function showToast(msg, color = "bg-red-700") {
    ...
    const type = color.includes("blue")
      ? "success"
      : color.includes("red")
      ? "error"
      : "info";
  ...
  async function onFileChange(e) {
    ...
    try {
      ...
      const newlyAddedDataUrls = await Promise.all(
        compressedFiles.map(fileToDataUrl),
      );
  ...
  function validateForm() {
    ...
    if (!form.price?.toString()?.trim())
      newErrors.price = t("form_required_price");
    ...
    if (!form.description?.trim())
      newErrors.description = t("form_required_description");
    if (previews.length === 0) newErrors.photos = t("form_required_photos");
    if (!location.district?.trim())
      newErrors.location = { district: t("form_required_district") };
    if (!location.taluka?.trim())
      newErrors.location = {
        ...newErrors.location,
        taluka: t("form_required_taluka"),
      };
    if (!location.village?.trim())
      newErrors.location = {
        ...newErrors.location,
        village: t("form_required_village"),
      };
    setErrors(newErrors);
      ...
      const listingPayload = {
        ...
        additional: Object.fromEntries(
          Object.entries(form).filter(
            ([key]) =>
              !["title", "quantity", "price", "description"].includes(key),
          ),
        ),
      };
  ...
  function handleLocationChange(updated) {
    ...
    setErrors((prev) => ({
      ...prev,
      location: {
        district: updated.district?.trim() ? "" : prev.location?.district || "",
        taluka: updated.taluka?.trim() ? "" : prev.location?.taluka || "",
        village: updated.village?.trim() ? "" : prev.location?.village || "",
      },
    }));
  }
  ...
  return (
    <>
      <form
        onSubmit={onSubmit}
        className={`space-y-6 p-4 sm:p-6 bg-white rounded-2xl shadow-lg dark:bg-[#1E1E1E] ${
          onClose ? "max-h-[80vh] overflow-y-auto" : ""
        }`}
      >
        {fields.map((f) => {
          const isRequired = [
        ...
        <div className="flex flex-col">
          ...
          {errors.photos && (
            <span className="text-red-500 text-sm mt-1">{errors.photos}</span>
          )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-3 relative">
            ...
            {previews.map((src, idx) => (
              <div
                key={idx}
                className="relative rounded-lg overflow-hidden border border-gray-200 shadow-sm group"
              >
