...
export default function CropDetailModal({ crop, onClose }) {
  ...
  return (
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 bg-black/80 backdrop-blur-md z-[9999] flex items-end md:items-center justify-center"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        onClick={onClose}
      >
        <motion.div
          className="bg-white dark:bg-[#121212] w-full md:w-[600px] h-[85vh] md:h-auto md:max-h-[85vh] rounded-t-[2rem] md:rounded-3xl overflow-hidden relative shadow-2xl flex flex-col"
          initial={{ y: "100%" }}
          animate={{ y: 0 }}
          exit={{ y: "100%" }}
