...
export default function MilkRateFilters({ onMilkTypeChange, onRegionChange, selectedMilkType, selectedRegion }) {
  ...
  const REGIONS = useMemo(() => [
    ...
    { value: "Uttar Pradesh", label: t("region_uttar_pradesh") || "Uttar Pradesh" },
  ...
  return (
    <div className="w-full">
      <div className="flex items-center gap-3 overflow-x-auto scrollbar-hide pb-2 px-1">
        {/* Milk Type Chips */}
        {MILK_TYPES.map((type) => {
          const isSelected = selectedMilkType === type.value || (type.value === "All" && !selectedMilkType);
          return (
            <button
              key={type.value}
        ...
        <div className="relative flex-shrink-0">
          <select
            ...
            className={`appearance-none pl-4 pr-8 py-2 rounded-full text-sm font-bold transition-all duration-300 border cursor-pointer focus:outline-none ${selectedRegion
                ? "bg-farm-600 text-white border-farm-600 shadow-lg shadow-farm-500/30"
                : "bg-white dark:bg-[#1E1E1E] text-gray-600 dark:text-gray-300 border-gray-200 dark:border-white/10 hover:border-farm-300"
              }`}
          >
            {REGIONS.map((region) => (
              <option key={region.value} value={region.value} className="bg-white text-gray-900 dark:bg-[#1E1E1E] dark:text-gray-200">
                {region.label}
              </option>
            ))}
          </select>
          <div className={`absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none ${selectedRegion ? 'text-white' : 'text-gray-500'}`}>
